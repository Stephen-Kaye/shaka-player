...
shaka.media.DrmEngine.prototype.prepareMediaKeyConfigsForVariants_ = function(
  ...
  for (const variant of variants) {
    ...
    for (const info of variant.drmInfos) {
      ...
      if (info.initDataTypes.length > 0) {
        config.initDataTypes = info.initDataTypes;
      }
...
shaka.media.DrmEngine.prototype.sendLicenseRequest_ = function(event) {
  ...
  if (this.currentDrmInfo_.keySystem.startsWith('com.microsoft.playready') ||
...
shaka.media.DrmEngine.fillInDrmInfoDefaults_ = function(
  ...
  if (advancedConfig) {
    ...
    if (advancedConfig.initDataTypes.length > 0) {
      drmInfo.initDataTypes = advancedConfig.initDataTypes;
    }
  }
